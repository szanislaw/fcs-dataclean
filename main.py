...
# === Recover UUID from original CSV by Variant Info ===
print("🔗 Matching UUIDs to final variants...")
uuid_df = pd.read_csv(csv_path, header=None)
uuid_df.columns = ["UUID", "Variant Info"]
variants_df = pd.read_excel(final_cleaned_output)
variants_df.columns = variants_df.columns.str.strip()
uuid_merged_df = pd.merge(variants_df, uuid_df, how='left', on='Variant Info')
uuid_merged_df = uuid_merged_df[["UUID"] + [col for col in uuid_merged_df.columns if col != "UUID"]]
uuid_merged_df.to_excel(final_uuid_output, index=False)
print(f"✅ Final file with UUIDs added saved as: {final_uuid_output}")
